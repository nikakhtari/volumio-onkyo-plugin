...
The shutdown helper uses a runtime config file generated by the plugin:

```text
/data/plugins/system_controller/onkyo_avr_manager/shutdown-poweroff-config.json
```

That file contains the currently configured or discovered AVR host and port. It exists because Volumio's plugin config manager may keep runtime settings outside the packaged `config.json`.
